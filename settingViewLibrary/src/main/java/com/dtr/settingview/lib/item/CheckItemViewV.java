...
public class CheckItemViewV extends FrameLayout {
	...
	private void readAttrs(Context context, AttributeSet attrs) {
		if (null != attrs) {
			...
			if (a.hasValue(R.styleable.SettingViewItem_setting_view_item_check)) {
				Drawable drawable = a.getDrawable(R.styleable.SettingViewItem_setting_view_item_check);
				if (null != drawable) {
			...
			if (a.hasValue(R.styleable.SettingViewItem_setting_view_item_background)) {
				Drawable drawable = a.getDrawable(R.styleable.SettingViewItem_setting_view_item_background);
				if (null != drawable) {
			...
			if (a.hasValue(R.styleable.SettingViewItem_setting_view_item_drawable)) {
				Drawable drawable = a.getDrawable(R.styleable.SettingViewItem_setting_view_item_drawable);
				if (null != drawable) {
			...
			if (a.hasValue(R.styleable.SettingViewItem_setting_view_item_title)) {
				String title = a.getString(R.styleable.SettingViewItem_setting_view_item_title);
				if (!TextUtils.isEmpty(title)) {
					mTitle.setText(title);
				}
			}

			if (a.hasValue(R.styleable.SettingViewItem_setting_view_item_titleColor)) {
				ColorStateList colors = a.getColorStateList(R.styleable.SettingViewItem_setting_view_item_titleColor);
				if (null != colors) {
					mTitle.setTextColor(colors);
				}
			}

			if (a.hasValue(R.styleable.SettingViewItem_setting_view_item_titleSize)) {
				int textSize = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_PX, a.getDimensionPixelSize(R.styleable.SettingViewItem_setting_view_item_titleSize, 16), getResources()
						.getDisplayMetrics());
				mTitle.setTextSize(textSize);
			}

			if (a.hasValue(R.styleable.SettingViewItem_setting_view_item_subTitle)) {
				String subTitle = a.getString(R.styleable.SettingViewItem_setting_view_item_subTitle);
				if (!TextUtils.isEmpty(subTitle)) {
					mSubTitle.setText(subTitle);
				}
			}

			if (a.hasValue(R.styleable.SettingViewItem_setting_view_item_subTitleColor)) {
				ColorStateList colors = a.getColorStateList(R.styleable.SettingViewItem_setting_view_item_subTitleColor);
				if (null != colors) {
					mSubTitle.setTextColor(colors);
				}
			}

			if (a.hasValue(R.styleable.SettingViewItem_setting_view_item_subTitleSize)) {
				int textSize = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_PX, a.getDimensionPixelSize(R.styleable.SettingViewItem_setting_view_item_subTitleSize, 12), getResources()
						.getDisplayMetrics());
				mSubTitle.setTextSize(textSize);
			}

			if (a.hasValue(R.styleable.SettingViewItem_setting_view_item_clickable)) {
				mItemViewContainer.setClickable(a.getBoolean(R.styleable.SettingViewItem_setting_view_item_clickable, true));
			} else {
				mItemViewContainer.setClickable(true);
			}

			if (a.hasValue(R.styleable.SettingViewItem_setting_view_item_checked)) {
				boolean isChecked = a.getBoolean(R.styleable.SettingViewItem_setting_view_item_checked, false);
				if (isChecked) {
					mCheck.setVisibility(View.VISIBLE);
				} else {
					mCheck.setVisibility(View.GONE);
